...
- Chunking uses a sliding window with overlap so we don't cut a sentence/idea
  in half at a chunk boundary and lose context.
- Embeddings use sentence-transformers (all-MiniLM-L6-v2): small, fast,
  runs locally/free, 384-dim vectors.
- FAISS IndexFlatIP (inner product) is used with L2-normalized vectors,
  which is mathematically equivalent to cosine similarity search.
- The LLM call uses Groq's free-tier API (OpenAI-compatible chat endpoint).
  Swap GROQ_API_URL / model name for any other provider (Gemini, OpenAI, etc.)
